...
?>
<?php
// Variables para almacenar temporalmente los datos ingresados por el usuario.
$nombre = '';
$usuario = '';
$correo = '';
...
if (isset($_POST['btn_enviar_R'])) {
    ...
    // Llamamos a la función para registrar el nuevo usuario
    $model->R_usuario = $_POST['R_usuario'];
    $model->R_contra = $_POST['R_contra'];
    $model->regNuevoUser();
}
...
<body style="background: rgb(1,5,36);
    ...
    ?>
            ...
            <input class="controls" type="text" maxlength="35" name="R_Nombre" value="<?php echo $nombre; ?>" oninput="this.value = this.value.toUpperCase();" onkeypress="return soloLetras(event);" placeholder="Ingrese su Nombre completo"><br>

            <h3>Nombre de Usuario</h3>
            <input class="controls" type="text" maxlength="15" name="R_usuario" value="<?php echo $usuario; ?>" onkeypress="return validarMayusculas(event)" onpaste="impedirPegar(event)" oninput="this.value = this.value.toUpperCase();" placeholder="Ingrese su Usuario"><br>
        <h3>Contraseña </h3>
        ...
        <input class="controls" type="password" maxlength="<?php echo $Max_Pass ?>" name="R_contra_2" id="R_contra_2" onkeypress="return bloquearEspacio(event)" onpaste="impedirPegar(event)" placeholder="Confirmar contraseña"><br>
