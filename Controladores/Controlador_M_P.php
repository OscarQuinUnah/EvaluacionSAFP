...
$sql=$conexion->query("SELECT * FROM `tbl_ms_usuario` WHERE Usuario='$User' and Estado_Usuario='ACTIVO'");
    ...
    if (mysqli_num_rows($sql)==0) {
     ...
     header( "refresh:0;url=../Pantallas/controlador_recupera_contra_p.php" ); 
        ...
        //CODIGO ANTERIOR SIN HASHEO DE RESPUESTAS
    ...
    if (mysqli_num_rows($sql1) >= 1) {
        ...
        if (password_verify($respuesta, $hashedResponseFromDB)) {
            ...
            echo '<script>alert("Pregunta o respuesta incorrecta.")</script>';
            header("refresh:0;url=../Pantallas/controlador_recupera_contra_p.php");
        }
    } else {
        // Pregunta o respuesta inválida, maneja el error
        $sql2 = $conexion->query("UPDATE tbl_ms_usuario SET Estado_Usuario='BLOQUEADO' WHERE ID_Usuario='$idUser'");
        echo '<script>alert("Pregunta o respuesta inválida. Usuario Bloqueado.")</script>';
        header("refresh:0;url=../Pantallas/controlador_recupera_contra_p.php");
    }
