    ...
    <script>
        $(document).ready(function() {
            window.addEventListener('beforeunload', function() {
                $.post('../../EVENT_BITACORA.php', { action: 'salirvinvol' });
            });
        });
    </script>
</body>
...
if ($datos=$sql->fetch_object()) { ?>
                ...
                <!-- Fin Generar Reporte -->
                    ...
                    <!-- /.box-header -->
                    <!-- centro -->
                    <?php $sql=$conexion->query("SELECT * FROM tbl_permisos where (Permiso_consultar=1 and ID_Rol=$ID_Rol and ID_Objeto=15) or ('$usuario'='ADMIN')");
if ($datos=$sql->fetch_object()) { ?>
...
if ($datos=$sql->fetch_object()) { ?>
                            <th></th>
                            <?php } ?>
                            <?php $sql=$conexion->query("SELECT * FROM tbl_permisos where (Permiso_Eliminacion=1 and ID_Rol=$ID_Rol and ID_Objeto=15) or ('$usuario'='ADMIN')");
if ($datos=$sql->fetch_object()) { ?>
                            <th></th>
                            <?php } ?>
                        </thead>
