...
if ($num_rows > 0) {
    while ($row = $resultado->fetch_assoc()) {
        ...
        $output['data'] .= '<td>' . $row['Pregunta'] . '</td>';
         ...
         $sql=$conexion->query("SELECT * FROM tbl_permisos where (Permiso_Actualizacion=1 and ID_Rol=$ID_Rol and ID_Objeto=4) or ('$usuario'='ADMIN')");
...
$sql=$conexion->query("SELECT * FROM tbl_permisos where (Permiso_Eliminacion=1 and ID_Rol=$ID_Rol and ID_Objeto=4) or ('$usuario'='ADMIN')");
if ($datos=$sql->fetch_object()) { 
