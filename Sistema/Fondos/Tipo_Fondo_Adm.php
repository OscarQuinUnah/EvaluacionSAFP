    ...
    <script>
        $(document).ready(function() {
            window.addEventListener('beforeunload', function() {
                $.post('../../EVENT_BITACORA.php', { action: 'salirtfondo' });
            });
        });
    </script>
              ...
              <div class="col-md-12">
                  <div class="box">
                    <div class="box-header with-border">
                          ...
                          <?php $sql=$conexion->query("SELECT * FROM tbl_permisos where (Permiso_Insercion=1 and ID_Rol=$ID_Rol and ID_Objeto=13) or ('$usuario'='ADMIN')");
if ($datos=$sql->fetch_object()) { ?>
                ...
                <!-- Fin Generar Reporte -->
                    ...
                    <!-- /.box-header -->
                    <!-- centro -->
                    <?php $sql=$conexion->query("SELECT * FROM tbl_permisos where (Permiso_consultar=1 and ID_Rol=$ID_Rol and ID_Objeto=13) or ('$usuario'='ADMIN') ");
if ($datos=$sql->fetch_object()) { ?>
...
document.getElementById("campo").addEventListener("keyup", function(event) {
  ...
  link.setAttribute("href", "../../fpdf/ReporteTipoFondos.php?campo=" + encodeURIComponent(campo));
});
  </script>
            <div class="row py-4">
                <div class="col">
                    <table class="table table-sm table-bordered table-striped">
                        <thead>
                            ...
                            <th class="sort asc">Nombre Fondo</th>
                          

                            <?php $sql=$conexion->query("SELECT * FROM tbl_permisos where (Permiso_Actualizacion=1 and ID_Rol=$ID_Rol and ID_Objeto=13) or ('$usuario'='ADMIN')");
if ($datos=$sql->fetch_object()) { ?>
                            <th></th>
                            <?php } ?>
                            <?php $sql=$conexion->query("SELECT * FROM tbl_permisos where (Permiso_Eliminacion=1 and ID_Rol=$ID_Rol and ID_Objeto=13) or ('$usuario'='ADMIN')");
if ($datos=$sql->fetch_object()) { ?>
                            <th></th>
                            <?php } ?>
                        </thead>
    ...
    <script>
        ...
        function getData() {
            ...
            let url = "Gestor_tbl_Tipo_de_Fondos.php"
