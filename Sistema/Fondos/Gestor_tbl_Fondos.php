...
if ($num_rows > 0) {
    while ($row = $resultado->fetch_assoc()) {
        ...
        $output['data'] .= '<td>' . $row['Fecha_de_adquisicion_F'] . '</td>';
         $sql=$conexion->query("SELECT * FROM tbl_permisos where (Permiso_Actualizacion=1 and ID_Rol=$ID_Rol and ID_Objeto=7) or ('$usuario'='ADMIN')");
if ($datos=$sql->fetch_object()) {
        $output['data'] .= '<td><a class="boton-editar" href="Update_Fondo.php?ID_de_fondo=' . $row['ID_de_fondo'] . '"><i class="zmdi zmdi-edit"></i></a></td>';
}
$sql=$conexion->query("SELECT * FROM tbl_permisos where (Permiso_Eliminacion=1 and ID_Rol=$ID_Rol and ID_Objeto=7) or ('$usuario'='ADMIN')");
if ($datos=$sql->fetch_object()) { 
